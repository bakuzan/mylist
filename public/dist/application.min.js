...
,S.findOne=function(){C.loading('editAnime',m.get({animeitemId:I}).$promise.then(function(x){S.animeitem=x,S.malSearchOptions.disabled=!1}))}// Find list of mangaitems for dropdown.
...
var g,h,p=s(new Date);return s(c).toISOString().substring(0,10)===s(u).toISOString().substring(0,10)?(g=s(u),h=g.fromNow(),'a day ago'===h)?'Yesterday at '+g.format('HH:mm'):-1<h.indexOf('days')?h+' at '+g.format('HH:mm'):h+'.':(g=s(c),h=p.diff(g,'days'),0===h)?'Today':1===h?'Yesterday':h+' days ago.'},maxCompleteMonth:function(c){var u={},p=0;for(var g=0;g<c.length;g++)if(c[g].end!==void 0&&null!==c[g].end){var h=c[g].end.substring(0,7);null===u[h]||u[h]===void 0?u[h]=1:u[h]++,u[h]>p&&(p=u[h])}return p},maxTagCount:function(c){var u={},p=0;return angular.forEach(c,function(g){angular.forEach(g.tags,function(h){var y=h.text;null===u[y]||void 0===u[y]?u[y]=1:u[y]++,u[y]>p&&(p=u[y])})}),p},ratingsWeighted:function(c){var u=[],p=[],g=0,h=0;/**
	             ...
	             */for(var w in p)'function'==typeof p[w]||isNaN(p[w])||(g+=w*p[w],h+=p[w]),0===w&&p[w];/* Weighted average.
							 ...
							 */return g/h},setSeason:function(c,u,p){var g=r('endedSeason')(c,u,p);return angular.forEach(g,function(h){h.season=d.convertDateToSeason(new Date(h.start))}),g}};return d}angular.module('animeitems').service('ItemService',t),t.$inject=['moment','$filter','ListService']}(),function(){'use strict';function t(s,r){//Retrieve the item values for grouping by
...
//                }
//            });
var d={name:l.name,show:function(){l.active=!0},hide:function(){l.active=!1},toggle:function(){l.active=!l.active}};r._register(d),l.$on('$destroy',function(){r._unregister(l.name)})}]}}angular.module('core').directive('loadingSpinner',t),t.$inject=['$http','spinnerService']}(),function(){'use strict';function t(s,r,l){return{restrict:'A',scope:{type:'=malSearch',selectItem:'=malSearchSelect',searchString:'=malSearchModel',options:'=malSearchOptions'},templateUrl:'/modules/core/templates/mal-search.html',bindToController:!0,controllerAs:'malSearchCtrl',controller:['$scope',function(d){function m(p,g){s.search(p,g).then(h=>{u.searchResults=h,u.hasSearchResults=!0,l.hide(u.spinner)})}function c(){u.hasFocus=!0,u.searchString!==void 0&&2<u.searchString.length&&null===u.selectedItem&&!u.options.disabled&&(l.show(u.spinner),m(u.type,u.searchString))}var u=this;u.delayInMs=1500,u.displayActions=!1,u.displaySelectedItemActions=function(){u.displayActions=!0},u.handleSearchString=c,u.hasFocus=!1,u.hasSearchResults=!1,u.processItem=function(g){u.selectItem(g),u.selectedItem=g},u.searchResults=[],u.selectedItem=null,u.selectedItemActions=[{displayText:'Remove selected',action:()=>{u.processItem(null),u.searchString=void 0,u.displayActions=!1}},{displayText:'Display raw json',action:()=>{u.displayRawJson=!0}}],u.spinner=`mal-search-${u.options.name}`,u.toggleSearchDropdownOnFocus=function(g){r(()=>{u.hasFocus='focus'===g.type,u.hasFocus&&0===u.searchResults.length&&u.searchString&&2<u.searchString.length&&c()},200)},d.$watch('malSearchCtrl.displayActions',p=>{p===void 0||p||(u.displayRawJson=p)})}]}}angular.module('core').directive('malSearch',t),t.$inject=['MalService','$timeout','spinnerService']}(),function(){'use strict';angular.module('core').directive('myProgress',function(){return function(s,r,l){s.$watch(l.myProgress,function(d){r.html('<div class="checklist-progress" style="width: '+d+'%;height: 100%"></div>')})}})}(),function(){'use strict';angular.module('core').filter('calendarFilter',function(){return function(s,r){if(s)return s.filter(function(l){//date filter
